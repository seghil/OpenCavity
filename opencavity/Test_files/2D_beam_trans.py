...
from beams import HGBeam
from propagators import FresnelProp
import numpy as np
import math
import matplotlib.pylab as plt

waist=100; waist_x=waist; waist_y=waist;
H=HGBeam(1,waist_x,waist_y) #creating the Hermite-Gauss basis with initial waist equals to 100 microns
z=0.000000000000000001

x=np.linspace(-6*waist, 6*waist,128); y=x
X,Y=np.meshgrid(x,y)
...
#for kx in range (n):
#    for ky in range (n):
#        if x[kx]==0 and y[ky]>0:
#            phase[kx,ky]=np.exp(1j*m/4)
#        elif x[kx]==0 and y[ky]<0:
#            phase[kx,ky]=np.exp(1j*3*m/4)
#            
#        elif x[kx]==0 and y[ky]==0:
#            phase[kx,ky]=np.exp(1j*3*m/4)    
#        
#        else:
#            phase[kx,ky]=np.exp(1j*m*(math.atan(y[ky]/x[kx])+(1-np.sign(x[kx])/2)*np.pi)/(2*np.pi))
#        
